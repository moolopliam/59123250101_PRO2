    ...
    $.slidey = function(el, options) {
        ...
        base.fetchSlides = function() {
            if (base.$el.find("ul").size() > 0) {
                base.$el.find("ul").first().children("li").each(function() {
                    ...
                    slide.description = ($this.find("p.description").size() > 0)
        ...
        base.constructLayout = function() {
            ...
            for (var slideIndex = 0; slideIndex < base.slides.length; slideIndex++) {
                ...
                elements += "<div class='slidey-list-thumbnail' style='background-image: url(\"" +
        ...
        base.addEventListeners = function() {
            base.layout.$list.on("click",
                "li",
                function() {
                    base.gotoSlide(base.layout.$list.find("li").index($(this)));
                });
            base.layout.$image.on("click",
                ".slidey-controls-previous",
                function() {
                    base.stopTimer();
                    base.slidePrevious();
                });
            base.layout.$image.on("click",
                ".slidey-controls-next",
                function() {
                    base.stopTimer();
                    base.slideNext();
                });
        ...
        base.gotoSlide = function(slideIndex) {
            ...
            base.layout.$overlay.fadeOut("fast",
                function() {
                    if (base.options.showList) {
        ...
        base.prepareNodes = function() {
            if (base.options.showNodes) {
                if (base.options.nodeContainer !== "" && $(base.options.nodeContainer).size() > 0) {
                    ...
                    for (var slideIndex = 0; slideIndex < base.slides.length; slideIndex++) {
                        var node = "<div class='slidey-node' style='background-image: url(\"" +
                            base.slides[slideIndex].image +
                            "\");'></div>";
                    ...
                    $(".slidey-node", base.layout.$nodeContainer).on("click",
                        function() {
