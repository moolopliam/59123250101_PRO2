...
(function(factory) {
        ...
        init: function(options) {
            return this.each(function() {
                ...
                }
                {
                    ...
                    // Initiate input
                    input.val("").data("inputpicker-uuid", uuid).addClass("inputpicker-input")
                        .prop("id", "inputpicker-" + uuid).prop("name", "inputpicker-" + uuid);

                    // Inputpicker div ( wrap fake input and arrow )
                    var inputpicker_div = $("<div id=\"inputpicker-div-" +
                            uuid +
                            "\" class=\"inputpicker-div\" data-uuid=\"" +
                            uuid +
                            "\" style=\"position:relative;overflow:auto;height:100%;\">" +
                            "<span class=\"inputpicker-arrow\" data-uuid=\"" +
                            uuid +
                            "\" onclick=\"$(this).parent().find('input').inputpicker('toggle');event.stopPropagation();\" style=\"" +
                            (input_disabled ? "display:none;" : "") +
                            "\"  ><b></b></span>" +
                            "</div>")
                        .append(input)
                        .on("click",
                    ...
                    var div_overflow_hidden =
                        $("<div class=\"inputpicker-overflow-hidden\" style='overflow: hidden;'></div>");
                    ...
                    // Original events
                    original.on("focus.inputpicker",
                        function() {
                            var original = $(this);
                            var input = _i(original);
                            input.trigger("focus");

                        }).on("change.inputpicker",
                        function() {
                            var original = $(this);
                            var input = _i(original);
                            _setValue(input, original.val());
                        }).on("disable.inputpicker",
                        function() {
                            var original = $(this);
                            var input = _i(original);
                            _disable(input, true);
                        }).on("enable.inputpicker",
                        function() {
                            var original = $(this);
                            var input = _i(original);
                ...
                var settings = $.extend({ cache: {} },
                    $.fn.inputpicker.defaults,
                    _options,
                    Array.isArray(options) ? { data: options } : options);
    ...
    function _execJSON(input, param, func, errorFunc) {
        ...
        if (_pagination(input)) {
            ...
            param[_set(input, "pageField")] = _set(input, "pageCurrent");
            param[_set(input, "pageLimitField")] = _set(input, "limit") ? _set(input, "limit") : 10;
        }
    ...
    function _initMultiple(input) {
        ...
        if (!inputpicker_div.find(".inputpicker-multiple").length) {
            ...
            input.detach().appendTo($("<li class=\"inputpicker-multiple-input\"></li>")
                .appendTo(inputpicker_multiple_div.find("ul")));
    ...
    function _initTag(input) {
        var inputpicker_div = _getInputpickerDiv(input);
        if (!inputpicker_div.find(".inputpicker-multiple").length) {
            var inputpicker_multiple_div =
                $("<div class=\"inputpicker-multiple\" style=\"\"><ul class=\"\"></ul></div>")
                    .prependTo(inputpicker_div);
            ...
            if (wrapped_elements.length && _getWrappedListElements(true).length == 0) {
                ...
                if (offset) { // Change active if necessary
                    if (offset < 0 && tr_active.prev().length) {
                        ...
                        if ((tr_active.next().position().top + 2 * tr_active.outerHeight()) >
                            wrapped_list.outerHeight()) {
                            wrapped_list.scrollTop(wrapped_list.scrollTop() + tr_active.outerHeight());
                        }
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            if (typeof filterField == "string" && filterField) // Search specific field
            {
                ...
                if (typeof filterField != "array" && typeof filterField != "object") {
                    filterField = [];
                    for (var k in fields)
                        filterField.push(typeof fields[k] == "object" ? fields[k]["name"] : fields[k]);
                }
    ...
    function _renderPaginationFooter(input) {
        ...
        if (fields.length > 0 && _pagination(input)) {
            ...
            output += "<div style=\"float:right;padding-right:5px;\">" +
                "<a href=\"javascript:void(0);\" onclick=\"$('#inputpicker-" +
                _uuid(input) +
                "').inputpicker('jumpToPage', '1');\">First</a>" +
                "<a href=\"javascript:void(0);\" onclick=\"$('#inputpicker-" +
                _uuid(input) +
                "').inputpicker('jumpToPage', '" +
                prev_page +
                "');\">Prev</a>" +
    ...
    function _dataRenderMultiple(input) {
        ...
        if (data.length) {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                output += '<tr class="inputpicker-element inputpicker-element-' +
                    i +
                    " " +
                    (isSelected ? "inputpicker-selected" : "") +
                    ' " data-i="' +
                    i +
                    '" data-value="' +
                    data[i][fieldValue] +
            ...
            output += "<thead><tr><td colspan='" +
                fields.length +
                "' align='center' class=\"inputpicker-no-result\">No results.</td></thead></tr>";
        }
    ...
    function _dataRenderDefault(input) {
        ...
        var tr_active = _getWrappedListElements(true);
        if (tr_active.length &&
            ((tr_active.position().top + 2 * tr_active.outerHeight()) > wrapped_list.outerHeight())) {
            wrapped_list.scrollTop(wrapped_list.scrollTop() + tr_active.data("i") * tr_active.outerHeight());
        }
    }
    ...
    function _setValue(input, value) {
        ...
        if (_typeIsMultiple(input)) {
            ...
            if (index_i > -1) {
                ...
                input.data("value",
                    data[index_i][fieldValue]); // Check if it is equal the original when selectMode is empty
    ...
    function _setValueForMultiple(input, value) {
        ...
        for (var i = 0; i < new_data.length; i++) {
            var d = new_data[i];
            $("<li class=\"inputpicker-element\" data-value=\"" +
                    d[fieldValue] +
                    "\"><span>" +
                    d[fieldText] +
                    "</span> <a href=\"javascript:void(0);\" onclick=\"$(this).closest('.inputpicker-div').find('input').inputpicker('removeValue', $(this).parent().data('value') );event.stopPropagation();\" onmouseover=\"$(this).prev().addClass();\" tabindex='-1'>x</a></li>")
                .insertBefore(li_input);
        }
    ...
    function _eventKeyDown(e) {
        ...
        case 8: // Backspace
            ...
            if (input.val() == "" &&
                (_typeIsMultiple(input) || _typeIsTag(input)) &&
                _getInputMultipleElements(input).length) {
                var original = _o(input);
    ...
    function _eventKeyUp(e) {
        ...
        if (_set(input, "url")) {
            ...
            delayHandler = setTimeout(_loadData,
                delay * 1000,
                input,
                function(input) {
                    _dataRender(input);
                    var wrapped_elements = _getWrappedListElements();
                    if (_isWrappedListVisible(input) &&
                        _getWrappedListElements(true).length == 0 &&
                        wrapped_elements.length) {
    ...
    $.fn.inputpicker = function(method) {
        ...
        if (typeof method == "object" || !method) {
            ...
            $.error("Method " + method + " does not exist on jQuery.inputpicker");
        }
    };
